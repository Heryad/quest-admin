...
export default function UsersPage({}: Props) {
  ...
  const addCategory = () => {
    setIsLoading(true);
    fetch('https://pear-trusting-femur.glitch.me/v2/newcategory', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded'
      },
      body: new URLSearchParams({
        'ctTitle': categoryName,
        'ctIcon': 'eded'
      })
    }).then(response => response.json())
      .then(data => {
      ...
      <div className="flex row-auto">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="sm:max-w-[500px]">
            ...
            <div className="flex row-auto">
              <div className="grid gap-4 py-4">
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Input id="name" className="col-span-3" value={categoryName} onChange={e => {setCategoryName(e.currentTarget.value)}}/>
                </div>
            ...
            <DialogFooter>
              <Button type="submit" onClick={() => {addCategory()}} disabled={isLoading}>{isLoading ? 'Please Wait ...' : 'Save changes'}</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
